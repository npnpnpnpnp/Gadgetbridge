...
def parse(b):
    ...
    for g in range(b[5]):
        ...
        for i in range(n):
            ...
            if kind in (0, 1):
                at += 1
            x, y, w, h = struct.unpack_from("<4H", b, at)
            at += 8
            count = b[at]
            at += 1
            offsets = struct.unpack_from("<" + "I" * count, b, at)
            at += 4 * count
...
if __name__ == "__main__":
    for path in Path(__file__).parent.glob("anim*.bin"):
        ...
        at, rs = parse(b)
        ranges = set()
        print(path.name, "header", at, "first image", min(o for r in rs for o in r[-1]))
        for r in rs:
            kind, bits, delay, x, y, w, h, offsets = r
            print("record", r[:-1], "frames", len(offsets))
            for off in offsets:
                size = w * h * (bits // 8)
                if bits == 8:
                    colors = struct.unpack_from("<H", b, off)[0]
                    size += 2 + 2 * colors
                    assert 0 < colors <= 256
