...
def convert(name, color, font):
    im = Image.new("RGB", (240, 286), color)
    out = io.BytesIO()
    im.save(out, format="BMP")
    bmp = out.getvalue()
    boundary = "FitproProbeBoundary7076"
    data = (
    ...
    req = urllib.request.Request(
        "https://tomato.gulaike.com/api/v1/convert/8bit",
        data=data,
        headers={
            "Authorization": token,
    ...
    with urllib.request.urlopen(req, timeout=30) as r:
        b = r.read()
    (root / (name + ".bin")).write_bytes(b)
    return name, len(b), b[:24].hex()
...
with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
    for result in pool.map(
        lambda a: convert(*a),
        [
            ("black-empty", "black", "empty.bin"),
            ("red-empty", "red", "empty.bin"),
            ("black-font", "black", "font.bin"),
        ],
    ):
